...
<%
    ...
    String now_category = "未分类";
    if (request.getAttribute("now_category") != null) {
        now_category = (String) request.getAttribute("now_category");
    }
    ...
    <nav aria-label="...">
        <ul class="pager">
            <%
                if (now_page > 1) {
            %>
            <li class="previous"><a href="CategoryPageServlet?category=<%=now_category%>&page=<%=now_page-1%>"><span
            ...
            <% if (now_page < page_amount) { %>
            <li class="next"><a href="CategoryPageServlet?category=<%=now_category%>&page=<%=now_page+1%>"> 下一页<span
